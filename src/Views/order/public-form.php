<div class="max-w-[600px] mx-auto px-6">
    <div
        class="bg-card-bg border border-border backdrop-blur-[16px] rounded-xl p-10 px-8 max-md:p-6 max-md:px-5 mt-10 mb-10">
        <h1 class="text-center text-[1.75rem] max-md:text-[1.4rem] font-bold mb-2 font-display">
            <?= __('order_catering') ?></h1>
        ...
        <form action="/order-form" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" id="name" name="name"
                    ...
                    placeholder="<?= __('enter_name') ?>" value="<?= \App\Core\View::e(old('name')) ?>" required>
            </div>
            ...
            <div class="mb-5">
                ...
                <div id="menu-items-container">
                    <div class="menu-item-row flex items-start gap-2" data-index="0">
                        ...
                        <button type="button"
                            class="remove-menu-item mt-1 w-9 h-9 flex items-center justify-center rounded-lg text-muted hover:text-danger hover:bg-danger/10 transition-all duration-150 cursor-pointer border-0 bg-transparent shrink-0 hidden"
                            data-index="0" title="<?= e(__('remove')) ?>">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="event_date"
                            class="block text-sm font-medium mb-1 text-muted"><?= __('event_date') ?> <span class="text-danger">*</span></label>
                        <input type="date" id="event_date" name="event_date"
                            class="w-full px-4 py-3 bg-white/5 border border-border rounded-xl text-text text-[0.95rem] outline-none transition-all duration-300 placeholder:text-white/20 focus:border-gold focus:ring-[3px] focus:ring-gold/20"
                            value="<?= \App\Core\View::e(old('event_date')) ?>" min="<?= date('Y-m-d') ?>" required>
                    </div>
                    <div>
                        <label for="event_time"
                            class="block text-sm font-medium mb-1 text-muted"><?= __('event_time') ?></label>
                        <input type="time" id="event_time" name="event_time"
                            ...
                            value="<?= \App\Core\View::e(old('event_time')) ?>">
                    </div>
                </div>
            </div>

            <div class="mb-5">
                <label for="occasion_select"
                    class="block text-sm font-medium mb-1 text-muted"><?= __('occasion') ?> <span class="text-danger">*</span></label>
                ...
                <input type="text" id="occasion_custom"
            ...
            <div class="mb-5">
                <label for="address"
                    class="block text-sm font-medium mb-1 text-muted"><?= __('delivery_address') ?></label>
            ...
            <div class="mb-5">
                <label for="notes" class="block text-sm font-medium mb-1 text-muted"><?= __('notes') ?> <span
...
$pageFlashes = [];
if ($flashError)
    $pageFlashes[] = ['type' => 'error', 'message' => $flashError];
?>
