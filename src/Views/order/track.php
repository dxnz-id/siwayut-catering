<div class="max-w-[540px] mx-auto px-6">
    <div
        class="bg-card-bg border border-border backdrop-blur-[16px] rounded-xl p-10 px-8 max-md:p-6 max-md:px-5 mt-16 mb-16">
        <h1 class="text-center text-[1.75rem] max-md:text-[1.4rem] font-bold mb-2 font-display">
            <?= __('track_order_title') ?>
        </h1>
        ...
        <form action="/track-order" method="POST">
            ...
            <div class="mb-5">
                <label for="order_number"
                    class="block text-sm font-medium mb-1 text-muted"><?= __('order_number') ?></label>
            ...
            <div class="mb-5">
                <label for="phone"
                    class="block text-sm font-medium mb-1 text-muted"><?= __('phone_when_ordering') ?></label>
                <input type="tel" id="phone" name="phone"
                    class="w-full px-4 py-3 bg-white/5 border border-border rounded-xl font-body leading-relaxed text-text text-[0.95rem] outline-none transition-all duration-300 placeholder:text-white/20 focus:border-gold focus:ring-[3px] focus:ring-gold/20"
                    placeholder="<?= __('phone_placeholder') ?>" value="<?= \App\Core\View::e(old('phone')) ?>"
                    required>
            </div>
